...
def main():
    ...
    parser.add_argument('--weights', default='/workspace/data/FFTformer/experiments/MFB_FFTformer_66/models/net_g_505000.pth', type=str, help='Path to weights')
    parser.add_argument('--dataset', default='SixFlow_FFTformer_580k_test', type=str, help='Test Dataset') # ['GoPro', 'HIDE', 'RealBlur_J', 'RealBlur_R']

    args = parser.parse_args()

    ####### Load yaml #######
    yaml_file = '/workspace/FFTformer/options/train/MFB_FFTformer.yml'
    import yaml
    ...
    files = []
    with open('/workspace/FFTformer/datasets/multi_flow_6/test_gopro.txt', 'r') as file:
        for line in file:
    ...
    with torch.no_grad():
        for idx,file_ in enumerate(tqdm(files, ncols=80)):
            ...
            scene = file_.split('/')[-3]
            blur_path = os.path.join(f'/workspace/data/Gopro_my25/test/{scene}/blur/{scene}.png')
            sharp_path = blur_path.replace('blur','sharp')

           
            flow = flow_utils.flow_read(file_)
            n_flow = []
            flows = [flow[..., i:i+2] for i in range(0, flow.shape[2], 2)]

            for f in flows:
                n_f = normalize_flow_to_tensor(f)
                n_flow.append(n_f)
            flow = np.concatenate(n_flow,axis=-1)  # H,W,15
            flow = torch.from_numpy(flow.transpose(2, 0, 1)).unsqueeze(0).float().cuda()  # [3,H,W]

            img = np.float32(utils.load_img(blur_path))/255.
            ...
            if idx % 2 == 1:
                ...
                save_path = os.path.join(result_dir, scene+'.png')
