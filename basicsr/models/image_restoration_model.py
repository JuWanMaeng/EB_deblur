...
class ImageRestorationModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        if local_rank == '0':
            ...
            wandb.run.name = 'MFB_FFTformer_6'
    ...
    def dist_validation(self, dataloader, current_iter, tb_logger, save_img, rgb2bgr, use_image):
        ...
        for idx, val_data in enumerate(dataloader):
            ...
            val_data['lq'] = torch.cat([val_data['lq'], val_data['flow']], dim = 1)
            val_data['lq'] = val_data['lq']
        ...
        if self.opt['rank'] == 0:
            ...
            for key, metric in zip(keys, metrics):
                if key == 'cnt':
                    cnt = float(metric)
                    continue
                metrics_dict[key] = float(metric) + 54.19   ####000101 scene 떄문에
