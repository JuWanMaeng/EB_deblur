...
class KLLoss(nn.Module):
    """KL Divergence Loss 클래스.
    
    Args:
        loss_weight (float): KL 손실의 가중치.
        reduction (str): 출력에 적용할 reduction 방식. 지원: 'none' | 'mean' | 'sum'
                         기본값은 'mean'입니다.
    """
    ...
    def forward(self, mu, logvar, weight=None, **kwargs):
        """Forward 메서드
        
        Args:
            mu (Tensor): 인코더에서 나온 평균 텐서.
            logvar (Tensor): 인코더에서 나온 로그 분산 텐서.
            weight (Tensor, optional): element-wise 가중치. 기본값: None.
        
        Returns:
            Tensor: 가중치가 적용된 KL Divergence Loss.
        """
        loss = kl_divergence_loss(mu, logvar, weight=weight, reduction=self.reduction)
