...
from basicsr.models.archs.NAFNet_ER_arch import NAFNet_ER
...
class EBR_fftformer(nn.Module):
    def __init__(self,
        ...
        super(EBR_fftformer, self).__init__()
        ...
        self.event_refinement = NAFNet_ER(img_channel=6, width=64,middle_blk_num=1,
                                       enc_blk_nums=[1, 1, 1, 28],
                                       dec_blk_nums=[1, 1, 1, 1],
                                       )
        
        self.event_refinement.load_state_dict(torch.load('/workspace/FFTformer/pretrained_model/ER.pth')['params'])
    ...
    def forward(self, y):
        ...
        # event refinement
        er_input = torch.cat([event,inp_img], dim=(1))
        event = self.event_refinement(er_input)
        max_val = torch.max(torch.abs(event))
        event = event / max_val  # [-1,1]
        inp = torch.cat([inp_img, event], dim=(1))
